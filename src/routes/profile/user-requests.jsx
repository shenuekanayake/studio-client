        ...
        <div className='container user__container'>
            {
                mode=="all"?<>
                {
                    users&&users.length>0&&users.map((user, index) => <>
                        ...
                        <div>
                            <button className='btn btn-primary' onClick={() => {updateUser(user, true); setNotApprovedUsers(notapprovedUsers.filter(item => item._id !== user._id)); setApprovedUsers(approvedUsers =>[...approvedUsers, user])}}>Accept</button>
                            |<button className='btn' onClick={() => {deleteUser(user); setApprovedUsers(approvedUsers.filter(item => item._id !== user._id)); setNotApprovedUsers(notapprovedUsers.filter(item => item._id !== user._id)); setUsers(users.filter(item => item._id !== user._id));}}>Remove</button>
                        </div>
                    </>)
                }
                </>:mode=="approved"?
                ...
                <>
                {
                    approvedUsers&&approvedUsers.length>0&&approvedUsers.map((user, index) => <>
                        ...
                        <div>
                            ...
                            <button className='btn btn-primary' onClick={() => {updateUser(user, false); setApprovedUsers(approvedUsers.filter(item => item._id !== user._id)); setNotApprovedUsers(notapprovedUsers =>[...notapprovedUsers, user])}}>Block</button>
                            |<button className='btn' onClick={() => {deleteUser(user); setApprovedUsers(approvedUsers.filter(item => item._id !== user._id)); setNotApprovedUsers(notapprovedUsers.filter(item => item._id !== user._id)); setUsers(users.filter(item => item._id !== user._id));}}>Remove</button>
                        </div>
                    </>)
                }
